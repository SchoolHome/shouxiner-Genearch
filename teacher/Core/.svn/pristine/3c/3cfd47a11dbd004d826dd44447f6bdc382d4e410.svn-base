...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	int(^createSocket)(int, NSData*) = ^int (int domain, NSData *interfaceAddr) {
        ...
        int testFlag = 0;
        testFlag = fcntl(socketFD, F_GETFL,0);
		status = fcntl(socketFD, F_SETFL, 0);
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	int testFlag = 0;
    testFlag = fcntl(childSocketFD, F_GETFL,0);
    int result = fcntl(childSocketFD, F_SETFL, 0);
...
- (void)didConnect:(int)aConnectIndex
{
	...
	int testFlag = 0;
    testFlag = fcntl(socketFD, F_GETFL,0);
    int result = fcntl(socketFD, F_SETFL, 0);
