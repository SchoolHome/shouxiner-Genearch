...
@implementation CPOperationMsgSend
...
-(void)sendGroupMsgText
{
    if (uiMsgGroup.groupServerID)
    {
        ...
        if (fromName&&toName)
        {
            ...
            NSNumber *newMsgID = [[[CPSystemEngine sharedInstance] msgManager] sendMsgWithMsgGroup:uiMsgGroup newMsg:uiMsg];
            [xmppMsg setMessageID:newMsgID];
            [[[CPSystemEngine sharedInstance] xmppEngine] sendGroupMessage:xmppMsg];
        }
    }
}
...
-(void)sendAutoMsgCoupleTeam
{
//    if ([uiMsgGroup msgGroupMemberRelation]==USER_MANAGER_FANXER)
//    {                        
//        CPDBModelMessage *newDbMsg = [[CPDBModelMessage alloc] init];
//        NSArray *oldMsgList = uiMsgGroup.msgList;
//        NSMutableArray *filterMsgArray = [[NSMutableArray alloc] init];
//        NSInteger oldMsgCount = [oldMsgList count];
//        for(int i=oldMsgCount-1;i>=0;i--)
//        {
//            CPUIModelMessage *oldMsg = [oldMsgList objectAtIndex:i];
//            if ([oldMsg.flag integerValue]==MSG_FLAG_SEND)
//            {
//                [filterMsgArray addObject:oldMsg];
//                if ([filterMsgArray count]==3)
//                {
//                    break;
//                }
//            }
...
//        if ([filterMsgArray count]<3)
//        {
//            [newDbMsg setMsgText:NSLocalizedString(@"AutoMsgShuangShuangAutoRes",nil)];
//        }
//        else 
//        {
//            CPUIModelMessage *oldMsg = [filterMsgArray objectAtIndex:0];
//            NSNumber *nowTime = [CoreUtils getLongFormatWithNowDate];
//            NSNumber *oldTime = oldMsg.date;
//            CPLogInfo(@"now time is %@, old time is %@",nowTime,oldTime);
//            if ([nowTime longLongValue]-[oldTime longLongValue]>=60)
//            {
//                [newDbMsg setMsgText:NSLocalizedString(@"AutoMsgShuangShuangIntuRes",nil)];
//            }
//            else 
//            {
//                [newDbMsg setMsgText:NSLocalizedString(@"AutoMsgShuangShuangAutoRes",nil)];
//            }
//        }
//        [[[CPSystemEngine sharedInstance] msgManager] sendAutoMsgTextWithMsgGroupID:uiMsgGroup.msgGroupID
//                                                                         newMsg:newDbMsg];
//    }
}
-(void)main
