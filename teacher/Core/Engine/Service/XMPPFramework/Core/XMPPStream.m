...
- (void)commonInit
{
	streamQueue = dispatch_queue_create("xmppStreamQueue", NULL);
...
- (id)init
{
	if ((self = [super init]))
	{
		...
		// Initialize socket
		asyncSocket = [(GCDAsyncSocket *)[GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:streamQueue];
	}
	return self;
}
...
{
	NSAssert(dispatch_get_current_queue() == streamQueue, @"Invoked on incorrect queue");
...
- (BOOL)connect:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if ([hostName length] == 0)
		{
			...
			srvResolver = [[XMPPSRVResolver alloc] initWithdDelegate:self delegateQueue:streamQueue resolverQueue:NULL];
...
- (BOOL)connectTo:(XMPPJID *)jid withAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		// Initailize socket
		asyncSocket = [[GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:streamQueue];
...
- (BOOL)connectP2PWithSocket:(GCDAsyncSocket *)acceptedSocket error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		[asyncSocket setDelegate:self delegateQueue:streamQueue];
...
- (void)sendIQ:(XMPPIQ *)iq withTag:(long)tag
{
	...
	else
	{
		...
		dispatch_async(concurrentQueue, ^{ @autoreleasepool {
			...
			dispatch_async(streamQueue, ^{ @autoreleasepool {
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
	...
	if (XMPP_LOG_RECV_PRE)
	{
		...
		XMPPLogRecvPre(@"RECV dataAsStr: %@", dataAsStr);
	}
...
- (void)xmppParser:(XMPPParser *)sender didReadRoot:(NSXMLElement *)root
{
	...
	dispatch_async(streamQueue, ^{ @autoreleasepool {
		...
		XMPPLogRecvPost(@"RECV root compactXMLString: %@", [root compactXMLString]);
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element
{
	...
	dispatch_async(streamQueue, ^{ @autoreleasepool {
		...
		XMPPLogRecvPost(@"RECV element compactXMLString: %@", [element compactXMLString]);
...
- (void)setupKeepAliveTimer
{
	...
	if (state == STATE_XMPP_CONNECTED)
	{
		...
		{
			keepAliveTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, streamQueue);
			...
			uint64_t interval = (((keepAliveInterval / 2.0)+1) * NSEC_PER_SEC);
...
- (void)testLink
{
    Reachability *reachability = [Reachability reachabilityWithHostName:@"chat1.ishuangshuang.com"];
...
- (void)keepAlive
{
	...
	if (state == STATE_XMPP_CONNECTED)
	{
		...
		{
            CPLogInfo(@"fire keepalive numberOfByteSend:%d",numberOfBytesSent);
