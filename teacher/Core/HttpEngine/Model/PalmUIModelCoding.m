...
+(id) deserializeModel : (NSString *)fileName{
    ...
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0];
    NSData *cacheData = [NSData dataWithContentsOfFile:[NSString stringWithFormat:@"%@%@%@",documentsDirectory,[NSString stringWithFormat:CachePath,account.uid],fileName]];
...
+(void) clearCache : (NSString *) fileName{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:[NSString stringWithFormat:@"%@%@%@",documentsDirectory,[NSString stringWithFormat:CachePath,account.uid],fileName]]) {
        [[NSFileManager defaultManager] removeItemAtPath:[NSString stringWithFormat:@"%@%@%@",documentsDirectory,[NSString stringWithFormat:CachePath,account.uid],fileName] error:nil];
    }
}
...
+(NSString *) CachePathDir{
    ...
    return [NSString stringWithFormat:@"%@%@",documentsDirectory,[NSString stringWithFormat:CachePath,account.uid]];
}
